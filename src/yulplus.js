...
var grammar = {
    ...
    {"name": "DTypeIdentifier", "symbols": [(lexer.has("DTIdentifier") ? {type: "DTIdentifier"} : DTIdentifier)], "postprocess": 
        function (d) {
    ...
    {"name": "DTypeMemoryStructIdentifier", "symbols": [(lexer.has("Identifier") ? {type: "Identifier"} : Identifier), "_", {"literal":":"}, "_", "DTypeIdentifier"], "postprocess": 
        function (d) {
          return {
    ...
    {"name": "DTypeMemoryStructDeclaration", "symbols": [{"literal":"dtmstruct"}, "_", (lexer.has("Identifier") ? {type: "Identifier"} : Identifier), "_", {"literal":"("}, "_", "DTypeMemoryStructList", "_", {"literal":")"}], "postprocess": 
        ...
        `,
                required: methodList.slice(0, i).filter((name, ind) => isDynamicArray(properties[ind]))
                  .map(name => name + '.size'),
              },
              [name + '.' + v.name + '.values.position']: {
                method: `
        ...
        `,
                required: isArray(v)
                  ? [
                      `${name + '.' + v.name}.length`,
                      name + '.' + v.name + '.' + v.value.dtype.inputs[0].label + '.encodeTight'
                    ]
                  : [name + '.' + v.name],
              },
            }
            ...
            if (isArray(v)) {
              ...
              methodMap = Object.assign(methodMap, buildMethodsProperty(
                name + '.' + v.name,
                subproperties,
                menthodL,
                Object.assign({}, acc, methodMap),
                dtproperty,
                i,
              ));
            }
            ...
            if (isNamedTuple(v)) {
              ...
              dtype.inputs.forEach((inp, ind) => {
                const dtproperty = {
                  type: 'DTypeIdentifier',
                  name: inp.label,
                  value: { dtype: dtypeutils.getByIdWithLength(inp.type) },
                }
                subproperties.push(dtproperty);
        ...
        `,
                required: components.map(inp => {
                  const req = [name + '.' + v.name + '.' + inp.name + '.encodeTight'];
                  if (isDynamicArray(inp)) req.push(name + '.' + v.name + '.' + inp.name + '.size');
                  return req;
